...
sys.stdout.write(f'\x1b[1;35m\x1b]2; ★ SCRIPT BY MR.TAKSU ★\x07')
...
class Main():
	...
	def intro(self):
		...
		my_r = my__.add(f"{c}Mr.Taksu{q}")
		my_rr= my_r.add(f"\r{k}My Github{q}")
		my_rr.add(f"{c}https://githuh.com/TaksuStore{q}")
		my_rr.add(f"{c}https://githuh.com/TaksuStore{q}")

		my_rrr=my_r.add(f"\r{i}My WhatsApp{q}")
		my_rrr.add(f"{b}6285891153455{q}")
...
def check_op(userrz,pwz,ttl,opsi):
	try:
		...
		if "c_user" in ses.cookies:
			opsi.add(f"{u} AKUNNYA KE SESI AWOKAWOKAWOK🤣🤣🤣{q}")
